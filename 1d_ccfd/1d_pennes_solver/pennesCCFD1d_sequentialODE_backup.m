function [U,xc,tGrid,V,S] = pennesCCFD1d_sequentialODE(domain,parameters,boundary,time,iterParm,odeParm)
...
%			boundary = updateBoundaryCoupledODE(boundary,L_sigma=L_sigma);
			...
			Q_bc  = computeRHSbc(tx,boundary,t,Uiter(1,:));

			% compute tensors 
			M_k   = computeStiffnessMatrix(tx);
			A_h   = computeMassMatrix(Por,dx);
			M_gam = computeMassMatrix(Gam,dx);
			M_bc  = computeBCMatrix(tx,boundary,t,Uiter(1,:)');  

			% construct LHS
			S = A_h + dt*M_k + dt*M_gam + dt*M_bc;

			% construct RHS
			Q = A_h*U(i-1,:)' + dt*M_gam*UStar + dt*F.*dx + dt*Q_bc;
