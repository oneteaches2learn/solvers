function result = mms_test_punctured(uTrue,domain,parameters,boundary,mmsParms,punctures)
...
%	punctures	cell array, entries are Puncture objects
%	
...
%-----------------------------------------------------------------------------%
	...
	% set up subdomain endpoints and BCtypes
	...
	% Manufacture Puncture BCs
	for i = 1:length(punctures)

		% get puncture endpoints and BCtypes
		a_i = punctures{i}.endpoint_L;
		b_i = punctures{i}.endpoint_R;
		leftType  = punctures{i}.BCtype_L;
		rightType = punctures{i}.BCtype_R;

		% manufacture boundary conditions
		data = manufacture_BCs(u,q,a_i,b_i,{0,leftType,0,rightType});
		punctures{i}.BC_L = data{1};
		punctures{i}.BC_R = data{3};

	end

	% Manufacture BCs for domain endpoints
	...
	for i = 1:trials,
		...
		[U{i},xc{i},tGrid{i}] = pennesCCFD1dtime_punctured({a,b,nx},parameters,boundary,{1/nt,nt,u_0},punctures);
		[X,Y] = meshgrid(xc{i},tGrid{i});
		V{i} = u(X,Y);
		Norm{i} = sqrt(sum(dx*dt*(U{i}-V{i}).^2,'all','omitnan')); % L2 error
	end;
	...
	if displayPlot == 1, 
		surfplotterCCFD(U{trials},{xc{trials},tGrid{trials}},dt,16);
 	end
