classdef GalerkinPennes2d_solver
	...
	methods
		...
		function self = solve(self)
			...
			for n = 1:time.M_t
				...
				% assemble tensors
				A   = self.assembleStiffnessMatrix(t);
				M_r = self.assembleMassMatrix(self.coefficients.r,t);
				M_p = self.assembleMassMatrix(self.coefficients.p,t);

				% update RHS and BCs
				b_vol = self.computeVolumeForces(t);
				U_D = self.computeDirichletBCs(t);
				b_neu = self.computeNeumannBCs(t);
				[E,b_rob] = self.computeRobinBCs(t);

				% final assembly
				v = sparse(self.domain.nNodes,1);
				S = dt * (A + M_r + E) + M_p;
				b = dt * (b_vol - b_neu + b_rob) - S * U_D + M_p * U(:,n);
		...
		function b = computeVolumeForces(self,t)
			...
			for j = 1:nElem3
				elementInd    = elements3(j,:);
				elementCoord  = coords(elementInd,:);
				xCoords = sum(elementCoord(:,1))/3;
				yCoords = sum(elementCoord(:,2))/3;
				b(elementInd) = b(elementInd) + ...
					det([1,1,1; elementCoord']) * ...
					(self.f(xCoords,yCoords,t) / 6 + ... 
					self.coefficients.r(xCoords,yCoords,t) * ...
					self.coefficients.uStar(xCoords,yCoords,t));
			end
