classdef GalerkinPoisson2d_mms
...
%-----------------------------------------------------------------------------%
	...
	methods
		function self = GalerkinPoisson2d_mms(dom,auxfun,NameValueArgs)
			arguments
				dom 	Domain2d
				auxfun  PoissonAuxFunctions2d
		...
		function problems = solveManufacturedProblems(self,demo)
			...
			base = 3; % <~~ at some point, need to pass this instead of hard-setting it
			if demo == 0, p_min = 1; p_max = 4; else p_min = demo; p_max = demo; end
			for p = p_min:p_max

				tic 
				dom_p = self.domain.setMesh(p,base);
				prob_p = GalerkinPoisson2d_solver(dom_p,cofs,f);
