...
for i = 1:loopsize(1), for j = 1:loopsize(2)
    ...
    % run test
    fprintf('TEST %d of %d. rxndiffMMS. uTrue: %s, BCs: %s\n', ...
                testNum,nTests,uTrue(i),bTypes_outer(j,:));
    %{
    try
        result(testNum) = rxndiffMMS_test(params,options);
    catch
        result(testNum) = 0;
    end
    %}
...
% Print summary
fprintf('SUMMARY for rxndiffMMS boundary condition test\n');
