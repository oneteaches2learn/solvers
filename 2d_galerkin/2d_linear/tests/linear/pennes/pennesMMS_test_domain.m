...
% coefficients
params.c = 2 + sin(x(1) + x(2)) * t + t;
params.k = 2 + sin(x(1) + x(2)) * t + t;
params.r = 2 + sin(x(1) + x(2)) * t + t;
params.uTrue = 2 + sin(x(1) + x(2)) * t + t;

% desired true solution
params.uTrue = cos(2 * pi * x(1)) * cos(2 * pi * x(2)) * t^2 + t^2;
...
for i = 1:loopsize(1), 
    ...
    % run test
    fprintf('TEST %d of %d. pennesMMS. domain: %s\n', ...
                testNum,nTests,str{i});
    result(testNum) = pennesMMS_test(dom_i,params,opt_i);
...
% Print summary 
fprintf('SUMMARY for pennesMMS domain test\n');
for i = 1:loopsize(1)
    if result(i) == 1
        passFail = 'PASS';
    else
