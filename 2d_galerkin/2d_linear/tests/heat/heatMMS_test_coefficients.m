...
for i = 1:loopsize(1), 
    ...
    % run test
    fprintf('TEST %d of %d. heatMMS. coefficients: c = %s, k = %s\n', ...
                testNum,nTests,c(i),k(i));
...
for i = 1:loopsize(1)
    allStrs{i} = sprintf(' coefficients: c = %s, k = %s.', c(i), k(i));
end
maxLen = max(cellfun(@length, allStrs));

% Print summary 
fprintf('SUMMARY for heatMMS coefficient test\n');
for i = 1:loopsize(1)
    if result(i) == 1
        passFail = 'PASS';
    else
