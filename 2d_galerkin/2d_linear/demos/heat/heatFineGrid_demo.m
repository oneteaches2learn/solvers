...
% FINE GRID TEST %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf('Heat Fine Grid Test Begun\n')
...
fprintf(' Contructing Domain:'), tic
	...
	dom = GalerkinAssembler2d_heat.assembleBoundary(dom,bTypes_outer,u_D,u_N,beta,u_R,bTypes_inner); 
	dom.time = TimeStepping(T,1);
executionTime = toc; 
fprintf(' %f s\n',executionTime)

fprintf(' Assembling Parameters:'), tic
	auxfun = GalerkinAssembler2d_heat.assembleCoefficients(c,k,0,f,uInit);
executionTime = toc; 
fprintf(' %f s\n',executionTime)

% run trial
fprintf(' Generating Solution:'), tic
	fine = GalerkinFineGrid2d_heat(dom,auxfun,mmsparams);
executionTime = toc; 
fprintf(' %f s\n',executionTime)
